...
edge1=[t([2 3],:);t([3 1],:);t([1 2],:)]; % 6 * num_t : 每两行是一条有向边的两个端点的节点标号

edge2=reshape(edge1,2,3*numt); %重排六行边为两行边，增加3倍列数
...
midp_in_t=reshape(ic,3,numt)+nump; % 第i行是所有单元的第i条边的中点标签，标签需要加上前面的顶点标签总数
